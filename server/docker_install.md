...
```sh
# Using new secure credentials
docker exec -it darkeden-mysql mysql -u denostaugia -p'denostaugia2024'
```

```SQL
use DARKEDEN;
update GameServerInfo set IP = 'YOUR_DROPLET_PUBLIC_IP';
update WorldDBInfo set IP = 'YOUR_DROPLET_PUBLIC_IP';
```

**Security Note**: The default credentials `elcastle`/`elca110` are publicly known and should NEVER be used. Always use unique, strong credentials!
