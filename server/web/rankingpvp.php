...
?>
	  ...
	  <div style="background:url(../images/servidor.png) no-repeat;"><div class='memin'><a href="ranking.php"><img src="images/ranking_top20.png" width="200" height="44" /></a></div><div class='memin'></div><br></br><br></br></span>
</div>
</div>
    <div id="content-middle">
    <div><?
echo "<center><table border='0' bordercolor='white' style='dashed thin'>";
echo "<tr><th  bgcolor=''><font color='#FFF'>WinName</th><th bgcolor=''><font color='#FFF'>WinLevel</th><th bgcolor=''><font color='#FFF'>LoserName</th><th bgcolor=''><font color='#FFF'>LoserLevel</font></th><th bgcolor=''><font color='#FFF'>Date</font></th></center>";

/* SQL QUERY COMMENTED OUT FOR VIEW MODE
$result = mysql_query("SELECT * FROM `KillerListInfo` ORDER BY `CreateDate` DESC")
or die(mysql_error());
*/
...
// keeps getting the next row until there are no more to get
		...
		while($row = mysql_fetch_array( $result )) {
			...
			$nombrew = $row['WinName'];
			$lvlw = $row['WinLevel'];
			$nombrel = $row['LoserName'];
			$lvll = $row['LoserLevel'];
			$fecha = $row['CreateDate'];
			$winrace = $row['WinRace'];
			$loserrace = $row['LoserRace'];
			echo "<tr><th bgcolor=''>";
			if($winrace == 4)
			{
				echo "<font color='#00CC33'>$nombrew</font>";
			} 
			if($winrace == 0)
			{
				echo "<font color='#0066CC'>$nombrew</font>";
			}
			if($winrace == 1)
			{
				echo "<font color='#993300'>$nombrew</font>";
			}
			echo "</td><th bgcolor=''>"; 
			echo "<font color='#60B3D2'>$lvlw</font>";
			echo "</td><th bgcolor=''>"; 
			if($loserrace == 4)
			{
				echo "<font color='#00CC33'>$nombrel</font>";
			} 
			if($loserrace == 0)
			{
				echo "<font color='#0066CC'>$nombrel</font>";
			}
			if($loserrace == 1)
			{
				echo "<font color='#993300'>$nombrel</font>";
			}
			echo "</td><th color=''>";
			echo "<font color='#60B3D2'>$lvll</font>";
			echo "</td><th color=''>";
			echo "<font color='#60B3D2'>$fecha</font>";
			echo "</td><th color=''>";
			$order1++;
		}
*/ 
