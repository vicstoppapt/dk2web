...
?>
<img src="images/parte.png">
<div style="background:url(../images/servidor.png) no-repeat;">
  <div class='memin'></div><div class='memin'></div><br></br><br></br></span>
</div>
</div>
	...
	if($user_id == "")
	{
		echo "
				<script language='javascript'>
				alert('Error: Usted debe loguearse primero')
				history.back(1)
			</script>
		";
	}
...
$query2 = mysql_query("SELECT * FROM `GoldMedalCount` WHERE `PlayerID` = '$user_id' ");
$query3 = mysql_query("SELECT * FROM `Slayer` WHERE `PlayerID` = '$user_id' AND `Active` != 'INACTIVE'  ");
...
/* DATABASE LOOP COMMENTED OUT FOR VIEW MODE
	...
	{      
      ...
      $Race = $row['Race'];
		if($Race == "VAMPIRE")
		{
			echo "<img src='config/images/grademarkvampire-0.bmp' title='Vampiro'></img>$char<p>";
		}
		if($Race == "OUSTER")
		{
			echo "<img src='config/images/grademarkousters-1.bmp' title='Ouster'></img>$char<p>";
		}
		if($Race == "SLAYER")
		{
			echo "<img src='config/images/grademarkslayer-0.bmp' title='Slayer'></img>$char<p>";
		}
    }
*/
	?>
      <a href="change_password.php" title="Change Password"><span class="form-button">Change Password</span></a></div>
    </p>
